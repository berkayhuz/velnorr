...
enum VelnorrTransition {
  ...
  static var panel: AnyTransition {
    // Keep media and utility panels on the same restrained horizontal motion.
    // Utility content uses this transition as well when switching panels.
    .opacity.combined(with: .offset(x: 4))
  }
