...
struct ExpandedNowPlayingView: View {
  ...
  let navigationDisplayMode: VelnorrDisplayMode
  let navigationCenterGap: CGFloat
  ...
  init(
    ...
    self.onOpenMirror = onOpenMirror
    ...
    self.navigationCenterGap = navigationCenterGap
...
let horizontalInset = min(
  ...
  geometry.size.width / 2
      ...
      let mediaContentOffset = isLockScreenWidget ? 0 : contentOffset

      ZStack(alignment: .topLeading) {
        if !isLockScreenWidget {
          VelnorrPanelNavigationView(
            displayMode: navigationDisplayMode,
            centerGap: navigationCenterGap,
            selectedPanel: .media,
            ...
            .frame(
              ...
              height: NotchMetrics.mediaNavigationHeight
            )
        }
  ...
  .offset(
    x: horizontalInset,
    y: isLockScreenWidget ? 16 : mediaContentOffset + NotchMetrics.mediaHeaderTopOffset
  )
...
if showProgress {
  ...
  .offset(
    x: horizontalInset,
    y: isLockScreenWidget
      ? 88
      : mediaContentOffset + NotchMetrics.mediaProgressTopOffset
  )
}
        ...
        controls
          ...
          .offset(
            ...
            y: isLockScreenWidget
              ? 120
              : mediaContentOffset + NotchMetrics.mediaControlsTopOffset
          )
      }
  ...
  private var expandedArtwork: some View {
    ZStack {
      if showArtwork, let artwork = status.artwork {
        if !motionReduced && status.isPlaying && !isLockScreenWidget {
