...
struct VelnorrShellView: View {
  ...
  @State private var gestureThresholdHapticTriggered = false
  ...
  var body: some View {
    ...
    let trackDetailsVisible = !interaction.hasExpandedPanel && (interaction.isArtworkHovered
    ...
    let batteryOverlayVisible =
      batteryHUDEnabled
      && notificationHUDEnabled
      && batteryCharge.isVisible && !interaction.hasExpandedPanel && !deviceOverlayVisible
    ...
    velnorrShape
      ...
      .overlay {
        ZStack {
          Group {
            ...
            .allowsHitTesting(!interaction.hasExpandedPanel)
            ...
            if interaction.isMediaExpanded {
              ...
              ExpandedNowPlayingView(
                ...
                navigationDisplayMode: metrics.displayMode,
                navigationCenterGap: metrics.centerGap,
                contentOffset: metrics.mediaContentOffset,
                onOpenCalendar: {
                  withAnimation(
                    reduceMotion ? .easeOut(duration: 0.12) : VelnorrAnimation.content
                  ) {
                    interaction.showUtility(.calendar)
                  }
                },
                onOpenShelf: {
                  withAnimation(
                ...
                onOpenMirror: {
                  withAnimation(
                    ...
                    interaction.showUtility(.mirror)
                  }
                }
              )
              .frame(
            ...
            } else if interaction.isWorkAreaExpanded,
              let selectedPanel = interaction.activePanel
            {

              VelnorrUtilityPanelView(
                runtime: runtime,
                selectedPanel: selectedPanel,
                displayMode: metrics.displayMode,
                centerGap: metrics.centerGap,
                contentHorizontalInset: min(layout.topRadius + 12, layout.width / 2),
                contentTopInset: metrics.utilityContentTopInset,
                onSelect: { panel in
                  withAnimation(
                    reduceMotion ? .easeOut(duration: 0.12) : VelnorrAnimation.content
                  ) {
                    interaction.showUtility(panel)
                  }
                },
                onOpenMedia: {
                  withAnimation(
              ...
              HStack(spacing: 0) {
                ...
                sideRegion(
                  ...
                  AlbumArtworkView(
                    ...
                    showsAmbientGlow: interaction.isMediaExpanded && music.status.isPlaying
                  )
      ...
      .scaleEffect(
        x: max(0.96, 1 + gestureProgress * 0.035),
        y: max(0.96, 1 + gestureProgress * 0.035),
        anchor: .top
      )
      .opacity(max(0.7, 1 - abs(gestureProgress) * 0.1))
      .simultaneousGesture(
        DragGesture(minimumDistance: 8, coordinateSpace: .local)
          .onChanged { value in
            handleVerticalGestureChange(value)
          }
          .onEnded { value in
            handleVerticalGestureEnd(value)
          }
      )
      .animation(
        animationsEnabled && !reduceMotion ? VelnorrAnimation.control : .easeOut(duration: 0.1),
      ...
      .onReceive(NotificationCenter.default.publisher(for: .velnorrOpenUtilityPanel)) { notification in
        guard !screenLock.isLocked,
          let displayID = notification.userInfo?["displayID"] as? CGDirectDisplayID,
          displayID == metrics.displayID,
          let rawValue = notification.userInfo?["panel"] as? String,
          let panel = VelnorrUtilityPanel(rawValue: rawValue)
        else { return }
        withAnimation(
          reduceMotion ? .easeOut(duration: 0.12) : VelnorrAnimation.hoverIn
        ) {
          interaction.showUtility(panel)
        }
      }
      ...
      .onReceive(
        ...
        withAnimation(reduceMotion ? .easeOut(duration: 0.12) : VelnorrAnimation.hoverIn) {
          if interaction.hasExpandedPanel {
            interaction.dismiss()
          } else {
            interaction.showMedia()
          }
      ...
      .onAppear {
        onMediaExpandedChange?(interaction.hasExpandedPanel)
      ...
      .onChange(of: interaction.activePanel) { panel in
        let expanded = panel != nil
  ...
  private func handleTrackChange(_ trackKey: String) {
    ...
    guard !interaction.hasExpandedPanel else { return }
  ...
  private func handleVerticalGestureChange(_ value: DragGesture.Value) {
    guard gesturesEnabled, !screenLock.isLocked else { return }
    guard !interaction.hasExpandedPanel || value.startLocation.y <= 70 else {
      gestureProgress = 0
      gestureThresholdHapticTriggered = false
      return
    }
    let translation = value.translation
    guard abs(translation.height) >= abs(translation.width) * 1.2 else { return }

    if interaction.hasExpandedPanel {
      guard closeGestureEnabled else {
        gestureProgress = 0
        gestureThresholdHapticTriggered = false
        return
      }
      gestureProgress = -VelnorrGesturePolicy.progress(
    ...
    if abs(gestureProgress) >= 1, !gestureThresholdHapticTriggered {
      performGestureHaptic()
      gestureThresholdHapticTriggered = true
    }
  }

  private func handleVerticalGestureEnd(_ value: DragGesture.Value) {
    defer {
      withAnimation(
        animationsEnabled && !reduceMotion ? VelnorrAnimation.control : .easeOut(duration: 0.1)
      ) {
    ...
    guard gesturesEnabled, !screenLock.isLocked else { return }
    guard !interaction.hasExpandedPanel || value.startLocation.y <= 70 else { return }
    let translation = value.translation
    guard abs(translation.height) >= abs(translation.width) * 1.2 else { return }

    if interaction.hasExpandedPanel {
      guard closeGestureEnabled,
      ...
      withAnimation(reduceMotion ? .easeOut(duration: 0.12) : VelnorrAnimation.hoverOut) {
        interaction.dismiss()
      }
    } else if VelnorrGesturePolicy.shouldTrigger(
      translation: translation.height,
      sensitivity: gestureSensitivity,
      direction: .down
    ) {
  ...
  private func performGestureHaptic() {
